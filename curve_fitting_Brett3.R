...
##Binder designation
snrv_logic=snr_vect2>=2
binder=all(snrv_logic[11:12])
binder6=all(snrv_logic[7:12])
...
##convert regr tables into one line and add the 95% CI
sub1=regr_tablog[1,1:4]
sub2=regr_tablog[2,1:4]

##error catch and calculation for 95%CI
CIerr=tryCatch(confint(reg1,"d"), error=function(e) e)
if(!inherits(CIerr, "error")){
CI=confint(reg1,"d")
}else{
CI=c("NA","NA")
}
...
## check for consecutive SNR threshold and bounding at high levels
boundhigh=is.na(CI[2])
if(boundhigh=="FALSE"&binder6=="TRUE"){
binderQ="TRUE"
}else{
binderQ="FALSE"
}

log_result=c(sub1,sub2,CI,binderQ)
}else{ 
...
##convert regr tables into one lineand add the 95% CI
sub1=regr_tablinear[1,1:4]
sub2=regr_tablinear[2,1:4]

##error catch and calculation for 95%CI
CIerr=tryCatch(confint(reg2,"d"), error=function(e) e)
if(!inherits(CIerr, "error")){
CI=confint(reg2,"d")
}else{
CI=c("NA","NA")
...
linear_result=c(sub1,sub2,CI)
}else{ 
linear_result="no convergence linear"
}

out=data.frame(name,ID,protein,snr_vect1,snr_vect2,binder,log_result)

#write.table(t(colnames(out)),"fitsheader.csv",sep=',',col.names=F,row.names=F,append=T)
write.table(out,"fits2.csv",sep=',',col.names=F,row.names=F,append=T)
